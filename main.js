...
class Tado extends utils.Adapter {
     ...
     * @param {string} caller
     */
    async apiCallOLD(url, method = 'get', payload = null, caller = '') {
        ...
        try {
            ...
            if (method !== 'get') {
                while (this.apiCallinExecution) {
                    this.debugLog(`API call in execution, waiting for... ${url}`);
            ...
            if (this.numberOfCalls.day == new Date().getDate()) {
                //same day
                this.numberOfCalls.calls = this.numberOfCalls.calls + 1;
            } else {
                //new day, reset counter
                this.log.info(`${this.numberOfCalls.calls} API calls at the end of yesterday.`);
                this.numberOfCalls.calls = 1;
                this.numberOfCalls.day = new Date().getDate();
            }
            if (this.numberOfCalls.calls % 20 === 0 && this.logCalls) {
                this.log.info(`${this.numberOfCalls.calls} API calls for today.`);
            }
            if (this.logCallsDetail) {
                this.log.info(`${this.numberOfCalls.calls} API calls for today. [${url}]`);
            }
        ...
        return undefined; // Should not be reached if error is thrown
    }
