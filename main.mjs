...
class Tado extends utils.Adapter {
    ...
    async DoData_Refresh(user, pass) {
        ...
        let step = 'start';
        if (now - this.lastupdate > oneHour)
            outdated = true;
        // Get login token
        try {
            step = 'login';
        ...
        if (method != 'get' && this.apiCallinExecution == true) {
            for (let i = 0; i < 10; i++) {
                ...
                if (this.apiCallinExecution != true) {
                    this.log.debug('Time to execute ' + url);
                    break;
                }
                else {
    ...
    async errorHandling(errorObject) {
        try {
            if (errorObject.message && (errorObject.message.includes('Login failed!') || errorObject.message.includes('ETIMEDOUT') || errorObject.message.includes('EAI_AGAIN') || errorObject.message.includes('No internet connection detected!')))
                return;
            if (this.log.level != 'debug' && this.log.level != 'silly') {
...
export default (options) => new Tado(options);
