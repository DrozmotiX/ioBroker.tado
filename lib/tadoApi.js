...
class TadoApi {
    ...
    async _doApiCall(url, method, payload, caller = '') {
        try {
            ...
            const rateLimitInfo = `Ratelimit Info: ${response.headers['ratelimit-policy']} | ${response.headers['ratelimit']}`;

            this._updateCallStats(url, rateLimitInfo);
     ...
     * @param rateLimitInfo
     */
    _updateCallStats(url, rateLimitInfo) {
